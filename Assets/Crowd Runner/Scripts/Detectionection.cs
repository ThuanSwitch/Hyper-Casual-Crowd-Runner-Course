...
public class PlayerDetection : MonoBehaviour
{
    ...
    [Header("Element")]
    [SerializeField] private CrowdSysterm crowdSysterm;
    ...
    {
        Collider[] detectedColliders = Physics.OverlapSphere(transform.position,crowdSysterm.GetCrowdRadius());
        for ( int i = 0 ; i < detectedColliders.Length; i++)
        {
            if (detectedColliders[i].TryGetComponent(out Doors doors))
            {
                ...
                onDoorsHit?.Invoke();
                doors.Disable();
                crowdSysterm.ApplyBonus(bonusType,bonusAmount);
            }
